...
# タンパク質配列を Ankh にかける際に、長いタンパク質についても分割して処理し、切断部周辺はいくつかスキップして重複部分は平均 Representation として
# 出力するスクリプト
# 
...
while len(fass) > 0 or len(remained) > 0:
    ...
    sequence_length = [];
    for dd in list(data):
        sequence_examples.append(list(dd[1]));
        sequence_length.append(len(dd[1]));
        sequence_names.append(dd[0]);

    ids = tokenizer.batch_encode_plus(sequence_examples, add_special_tokens=True, padding=True, is_split_into_words=True, return_tensors="pt");
    input_ids = ids['input_ids'].to(ddev);
    attention_mask = ids['attention_mask'].to(ddev);
