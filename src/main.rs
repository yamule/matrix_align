...
impl FreqSeqAligner {
    ...
    pub fn calc_swapped_score(&self,seqa:&FreqSequence
        ,targetid:i32
        ,seqb:&FreqSequence,mapper:&Vec<(i32,i32)>)-> f32{
        assert_eq!(seqb.get_num_seq(), 1);
        let mut ret = 0;
        let target_uid = seqa.primary_ids.iter().position(|&e| e == targetid).unwrap_or_else(||panic!("{} can not find??? {:?}",targetid,seqa.primary_ids));

        for a in mapper.iter(){
            let mut counter:Vec<i32> = vec![0;NUM_CHARTYPE];
            let mut _maxindex:usize = 0;
            let mut maxcount:i32 = 0;
            if a.0 > -1{    
                for (ee,vv) in self.letters_colget(seqa.letbuff_id as usize,a.0 as usize).iter().enumerate(){
                    counter[ee] += *vv;
                }
                counter[
                    self.charmap[    
                        self.ali_get(seqa.alibuff_ids[target_uid],a.0 as usize) as usize
                    ]
                ] -= 1;
            }else{
                counter[self.charmap['-' as usize]] += seqa.get_num_seq() as i32;
            }
            if a.1 > -1{
                counter[
                    self.charmap[    
                        self.ali_get(seqb.alibuff_ids[0],a.1 as usize) as usize
                    ]
                ] += 1;
            }else{
                counter[self.charmap['-' as usize]] += 1;
            }
            let mut allcount = 0;
            for ee in 0..NUM_CHARTYPE{
                allcount += counter[ee];
                if maxcount < counter[ee]{
                    maxcount = counter[ee];
                    _maxindex = ee;
